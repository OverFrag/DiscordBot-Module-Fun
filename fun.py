import random
import discord
from discordbot.bot import Container
from discordbot.module import ModuleHelper
...
def init(container: Container):
    helper = ModuleHelper(container)

    @container.client.event
    async def on_message(message: discord.Message):
        if helper.is_my(message):
            return False

        chance = random.randint(0, 100)

        if random.randint(0, 500) is 5:
            await container.client.send_message(message.channel, random.choice(chuck))

        for key in messages:
            if key.lower() in message.content.lower() and chance >= get_chance(key.lower()):
                await container.client.send_message(message.channel, random.choice(messages[key]))
                break

    def get_chance(key):
        if key in chances:
            return 100 - chances[key]
        else:
            return 0


def admin(container: Container):
    return [
        ('/', Index)
    ]
