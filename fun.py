...
class Fun(Module):
    ...
    async def on_message(self, message: discord.Message):
        ...
        if random.randint(0, 500) is 5:
            await self.container.client.send_message(message.channel, random.choice(self.chuck))

        for key in self.messages:
            if key.lower() in message.content.lower() and chance >= self.get_chance(key.lower()):
                await self.container.client.send_message(message.channel, random.choice(self.messages[key]))
                break
    ...
    def get_chance(self, key):
        if key in self.chances:
            return 100 - self.chances[key]
        else:
            return 0


class FunIndex(Resource):
    def get(self):
        return {
            'chances': Fun.chances,
